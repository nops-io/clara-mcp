...
Always produce **one single artifact** after presenting query results — a chart
communicates trends and breakdowns far more clearly than a table alone.

**In Claude:** Render one React artifact (using Recharts or a plain SVG). Use a bar
chart for categorical breakdowns, a line chart for time series, and a stacked bar for
multi-dimension comparisons. Pass the query result rows directly as data — do not
hardcode values. Label axes and add a legend when there are multiple series.

**In Cursor:** Create a single canvas with a chart that visualizes the data. Use
Cursor's canvas feature to render it inline. Apply the same chart-type guidance above
(bar for categories, line for time series, stacked bar for multi-dimension).
